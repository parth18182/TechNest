...
function ImageUpload({ productData, setProductData }) {
    ...
    return (
        <div className='grid gap-2'>
            <Label>Product Imgaes</Label>
            <Input type={'file'} id="file-upload" className={'hidden'} accept="image/*" multiple onChange={handleFiles} />
            <Button variant='outline'><label htmlFor="file-upload" className='cursor-pointer'>Upload Images</label></Button>
                    ...
                    <div className='grid grid-cols-2 gap-4 mt-3 sm:grid-cols-3'>
                        {
                            productData.productImg.map((file, index) => {
                                //Check is file is a already file (from input) or db string
